...
class image_Dataset(Dataset):
    def __init__(self, image_paths, target_paths, use_flip=True, target_th=0.2):  # initial logic happens like transform
        ...
        self.do_rand_hflip = RandomHorizontalFlip()
        self.do_rand_vflip = RandomVerticalFlip()
        
        self.do_img_transforms = T.Compose([
            T.ToTensor(),
            # T.Normalize(mean=1, std=1),
            # T.Pad(8, fill=0),
            # T.Lambda(lambda x: torch.cat([x, x, x], 0)),
        ])
        self.do_msk_transforms = T.Compose([
            T.ToTensor(),
    ...
    def get_posedata(self, img, msk, num_ch=11):
        ...
        msk_uint = np.uint8(np.where(msk>self.target_th, 1, 0))
        
        num_labels, labels_im = cv2.connectedComponents(msk_uint)
        # num_labels, labels_im, states, centers = cv2.connectedComponentsWithStats(msk_uint)
    ...
    def transform(self, image, mask):
        ...
        image_ahe1 = skimage.exposure.equalize_adapthist(image, clip_limit=0.01)
        image_ahe2 = skimage.exposure.equalize_adapthist(image, clip_limit=0.01, kernel_size=13)
        image = np.stack([image_ahe1, image_ahe2, image], -1)
        # image = np.expand_dims(image, -1)
        
        if self.use_flip:
            # Random horizontal and vertical flipping
            image, mask = self.do_rand_hflip(image, mask)
            image, mask = self.do_rand_vflip(image, mask)
        
        # # Transform to tensor
        # image, mask = ToTensor()(image, mask)
        image = self.do_img_transforms(image.copy()).float()
        mask = self.do_msk_transforms(normalize(mask.copy())).float()
...
def save_attention(inputs, outputs, targets, att, target_th=0.5, save_dir="./visualize", id=""):
    ...
    att = normalize(torch.sigmoid(att).numpy())
    ...
    clr_vis_Y = [x.transpose([1,2,0])*255 for x in inputs]
    ...
    for idx, Y in enumerate([targets, outputs]):
        ...
        for y, x in zip(Y, inputs):            
            ...
            for ych, hue_i in zip(y, hues):
                ...
                if idx>=0:
                    ych = cv2.GaussianBlur(ych,(9,9),cv2.BORDER_DEFAULT)
    ...
    clr_vis_Y = [np.rot90(t) for t in clr_vis_Y]
    
    t = np.array(clr_vis_Y)
    t = np.transpose(t, [0, 3, 1, 2])
    trgts = make_grid(torch.Tensor(t), nrow=4)
    txt = f'{save_dir}/{id}.png'
    res = np.transpose(trgts.numpy(), (1,2,0))
    cv2.imwrite(txt, res)
