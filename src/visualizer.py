...
class Visualizer(object):
    def __init__(self,):
        pass
    def load(self, im, target, coord_gt, outs, ipses, gatt):
        self.im = im.detach().cpu().numpy().transpose([1, 2, 0])
        self.gt = target.detach().cpu().numpy()
        if coord_gt:
            self.coord_gt = np.rot90(np.array(coord_gt), 3)/319
        else:
            self.coord_gt = []
        self.outs = outs
        self.ipses = ipses
    ...
    def __plot_vertebral(self, ax, mode, joints, vis, size, im=None, color=None):
        h,w,_=im.shape if im else self.im.shape
        j=joints
        for i in range(len(j)):
            c = color if color else self.colors[i]
            if not vis[i]: continue
            if "point" in mode.lower():
                ax.plot(j[i,1]*w, j[i,0]*h, 'o', color=c, markersize=size, alpha=1)
            else:  
                if i:
                    ax.plot(j[i-1:i+1,1]*w, j[i-1:i+1,0]*h, color=c, linewidth=size, alpha=1)   
    
    def get_overlayed_vertebral(self, joints, im, vis):
        h,w,_=im.shape
        j=joints
        fig, ax = plt.subplots(figsize=(10, 10))
        ax.imshow(im, cmap="gray")
        if not isinstance(self.coord_gt, list):
            vis[len(self.coord_gt):]=0
            # self.__plot_vertebral(ax, "line", self.coord_gt, vis, size=3, color=(0,1,0))
            self.__plot_vertebral(ax, "point", self.coord_gt, vis, size=9, color=(0,1,0))
            self.__plot_vertebral(ax, "point", j, vis, size=7, color=(1,0,0))
        else:
            self.__plot_vertebral(ax, "line", j, vis, size=3)
            self.__plot_vertebral(ax, "point", j, vis, size=8)
        
        plt.axis("off")
        fig.tight_layout()
        fig.canvas.draw()
        X = np.array(fig.canvas.renderer.buffer_rgba())[:,:,:-1]
        
        if not isinstance(self.coord_gt, list):
            gt = self.coord_gt*len(X)
            pad = int(0.03*len(X))
            min_r = max(int(gt[:,0].min())-pad, 0) 
            max_r = min(int(gt[:,0].max())+pad, X.shape[0])
            min_c = max(int(gt[:,1].min())-pad, 10) 
            max_c = min(int(gt[:,1].max())+pad, X.shape[1])
            h, w = int(max_r - min_r), int(max_c - min_c)
            
            cr_X = X[min_r:max_r, min_c:max_c]
            scale = X.shape[1]/cr_X.shape[1] - 0.05
            size = int(cr_X.shape[1]*scale), int(cr_X.shape[0]*scale)
            cr_X = cv2.resize(cr_X, size)
            b = 10
            cr_X[:b, :] = X.max()//1.5; cr_X[-b:, :] = X.max()//1.5
            cr_X[:, :b] = X.max()//1.5; cr_X[:, -b:] = X.max()//1.5
            cr_X[:b, :, 2] = 0; cr_X[-b:, :, 2] = 0
            cr_X[:, :b, 2] = 0; cr_X[:, -b:, 2] = 0
            
            ddd = int(0.8*X.shape[0])
            X[:ddd, :] = X[-ddd:, :]
            X[ddd:, :] = X.max()
            # X[-cr_X.shape[0]-2:-cr_X.shape[0], :]=X.max()
            X[-cr_X.shape[0]:, :cr_X.shape[1]] = cr_X
            m = 30
            Y = np.zeros((X.shape[0]+2*m, X.shape[1]+2*m, X.shape[2]), dtype=X.dtype)+255
            Y[m:-m, m:-m, :] = X
        else:
            Y = X
        Y = np.rot90(Y, 1)
    ...
    def _get_overlayed_on_img(self, att):
        att = cv2.applyColorMap(att, cv2.COLORMAP_JET)[:,:,::-1]
        res = self.im*255*0.5+ att*0.5
        res = np.rot90(np.uint8(res))
        res = cv2.resize(res, [512, 512])
        return res

    def get_hca_atts(self):
        # out_atts = [self._get_overlayed_on_img(np.uint8(255*normalize(F.sigmoid(o[0]).sum(dim=0).detach().cpu().numpy()))) for o in self.outs]
        # outs = np.uint8(255*normalize(np.concatenate(out_atts, 1)))
        out_att = np.uint8(255*normalize(F.sigmoid(self.outs[-1][0]).sum(dim=0).detach().cpu().numpy()))
        out = np.uint8(255*normalize(self._get_overlayed_on_img(out_att)))
        return out
    
    def get_ips_atts(self):
        ips_atts = []
        vertebrals = []
        ips = self.ipses[-1]
        
        att, m_joints, v_joints, vis = ips
        j = m_joints[0].detach().cpu().numpy()
        _ips_att = np.uint8(255*normalize(F.sigmoid(att[0]).sum(dim=0).detach().cpu().numpy()))
        
        ips_att = np.uint8(255*normalize(self._get_overlayed_on_img(_ips_att)))
        vertebral = self.get_overlayed_vertebral(j, self.im, vis[0])
    
        return ips_att, vertebral
...
def save_epoch_res_as_image(inputs, targets, epoch_num, flag_gt):
    ...
    for y, x in zip(targets, inputs):
        ...
        txt = ''
        if flag_gt:
            txt = f'vis/c-tr/epo_{epoch_num:3d}_gt.png'
        else:
            txt = f'vis/c-tr/epo_{epoch_num:3d}_pr.png'
...
def save_epoch_res_as_image2(inputs, outputs, targets, epoch_num, target_th=0.4, pretext=False, save_dir="./visualize"):
    max_epoch = 500
    ...
    if pretext:
        txt = f'{save_dir}/{epoch_num:0=4d}_test_result.png'
    else: 
        txt = f'{save_dir}/epoch_{epoch_num:0=4d}_res2.png'
...
import copy
def save_attention(inputs, outputs, targets, att, target_th=0.5, save_dir="./visualize", id=""):
    targets = targets.data.cpu().numpy()
    outputs = outputs.data.cpu().numpy()
    inputs  = inputs.data.cpu().numpy()
    ...
    firsts = [0, 1]
    for idx, Y in enumerate([targets, outputs]):
        x_bool_first = True
        for y, x in zip(Y, inputs):
            y_colored = np.zeros([y.shape[1], y.shape[2], 3], dtype=np.uint8)
            y_all = np.zeros([y.shape[1], y.shape[2]], dtype=np.uint8)
            for ych, hue_i in zip(y, hues):
                ...
                if idx>=0:
                    ych = cv2.GaussianBlur(ych,(7,7),cv2.BORDER_DEFAULT)
                    ych = np.where(ych>0.05, ych, 0)
            ...
            if x_bool_first: # input image
                if firsts.pop() == 0:
                    clr_vis_Y.append(np.uint8(x_3ch)*0.75)
                else:
                    clr_vis_Y.append(rgbatt)
                x_bool_first = False
            
            x_3ch = np.where(y_colored>0, 0.0*x_3ch, x_3ch)
            img_mix = np.uint8(x_3ch + y_colored*0.99)
            # img_mix = cv2.cvtColor(img_mix, cv2.COLOR_BGR2RGB)
            clr_vis_Y.append(img_mix)    
    ...
    t = np.transpose(t, [0, 3, 1, 2])
    trgts = make_grid(torch.Tensor(t), nrow=4)
    txt = f'{save_dir}/attention_visualization_{id}.png'
    res = np.transpose(trgts.numpy(), (1,2,0))
    cv2.imwrite(txt, res)
...
def get_att_layer_output(model):
    save_output = SaveOutput()
    for layer in model.modules():
        if isinstance(layer, torch.nn.modules.conv.Conv2d):
            if layer.weight.size()[0]==11:     
